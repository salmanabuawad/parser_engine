...
- **Block mapping file (3rd upload)**: User can optionally upload a third file (Excel/CSV or PDF with table) that maps design blocks → execution blocks. Design blocks may split or merge into execution blocks, and the mapping includes which trackers/rows belong to which execution block. When this file is present, the parser should ignore PDF-derived block boundaries and reassign all piers/trackers to execution blocks per the mapping. Needs: new upload `kind` ("block_mapping"), parser logic to read mapping and override block assignments, support for both Excel/CSV and PDF table formats (auto-detect by extension).
