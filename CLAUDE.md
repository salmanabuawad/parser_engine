...
- **Block mapping file (3rd upload)**: User can optionally upload a third file that maps design blocks → execution blocks. Design blocks may split or merge into execution blocks, and the mapping includes which trackers/rows belong to which execution block. When this file is present, the parser should ignore PDF-derived block boundaries and reassign all piers/trackers to execution blocks per the mapping. Supported formats: Excel/CSV, PDF with table, or **image** (PNG/JPG — use OCR or CV to extract block boundaries/labels from the image). Auto-detect by extension. Needs: new upload `kind` ("block_mapping"), parser logic to read mapping and override block assignments.
